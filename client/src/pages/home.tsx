...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Always send a full main style + 2 sub styles — the server mixes them
    // 2:1:1 so the feed has variety and changes on refresh
    const PAD_TOPS: Record<string, string[]> = {
      male:   ["Streetwear", "Vintage", "Old Money"],
      female: ["Vintage", "Minimalist", "Y2K"],
      both:   ["Streetwear", "Vintage", "Minimalist"],
    };
    for (const d of PAD_TOPS[genderPref] ?? PAD_TOPS.both) {
      if (tops.length >= 3) break;
      if (!tops.includes(d)) tops.push(d);
    }
